...
/**
 * Install / refresh Pictoer ComfyUI custom nodes (direct multi-codec save).
...
/** Pictoer layout: `{download}/python/venv/Scripts/python.exe` + `{download}/python/tools/uv.exe`. */
 ...
 * against Pictoer's torch 2.9.1+cu128 (Windows 0xc0000139 / CDLL error).
 */
  ...
  if (!(await torchAudioLoads(pythonExe))) {
    throw new Error(
      `torchaudio still fails to load after aligning to torch ${torchVer}. ` +
        `Try reinstalling Pictoer Python, then ComfyUI Download again.`
    )
  }
}
...
async function installComfyRequirements(
  ...
  if (!(await hasPipModule(pythonExe))) {
    ...
    try {
      ...
      throw new Error(
        `This Python has no pip, and ensurepip failed. Use Pictoer Python Download (uv) or install pip manually. ${detail}`
      )
    }
  }
...
export async function writeExtraModelPaths(
  ...
  const lines: string[] = ['# Generated by Pictoer — do not edit while app is managing ComfyUI', '']
...
export async function installComfyUi(opts: {
  ...
  try {
    ...
    onProgress?.({
      stage: 'pip',
      message: 'Installing ComfyUI requirements (using Pictoer Python)…',
      pct: 45
    })
